...
// According to LRO conventions, LRO methods always use the field name "name" to reference
// the target resource or parent collection.
...
var standardInflections = map[string]string{
...
func InferOperationResource(m *api.Method) (*api.Resource, error) {
	...
	inflections := BuildInflectionMap(m.Model)
	var patterns []api.ResourcePattern
	for _, binding := range m.PathInfo.Bindings {
		expanded, err := expandBinding(binding, inflections)
		if err != nil {
			return nil, err
		}
		if len(expanded) > 0 {
...
func expandBinding(binding *api.PathBinding, inflections map[string]string) ([]api.PathSegment, error) {
	...
	for _, seg := range binding.PathTemplate.Segments {
		if seg.Literal != nil {
			...
			expanded, err := expandNestedVariable(seg.Variable, inflections)
			if err != nil {
				return nil, err
			}
...
func expandNestedVariable(v *api.PathVariable, inflections map[string]string) ([]api.PathSegment, error) {
	...
	for i, part := range v.Segments {
		if part == "*" || part == "**" {
			...
			singular, ok := inflections[plural]
			if !ok {
				singular, ok = standardInflections[plural]
			}
			if !ok {
				return nil, fmt.Errorf("failed to resolve singular resource name for plural collection literal %q: no matching pattern found in API model resource definitions", plural)
			}
